...
class Quaternion:
    ...
    def to_euler_angles(self):
        ...
        if np.abs(self[1] * self[2] + self[3] * self[0] - 0.5) < 1e-8:
            ...
            roll = np.arctan2(
                2 * self[0] * self[1] - 2 * self[2] * self[3],
                1 - 2 * self[1] ** 2 - 2 * self[3] ** 2,
            )
            yaw = np.arctan2(
                2 * self[0] * self[2] - 2 * self[1] * self[3],
    ...
    def to_euler123(self):
        roll = np.arctan2(
            -2 * (self[2] * self[3] - self[0] * self[1]),
            self[0] ** 2 - self[1] ** 2 - self[2] ** 2 + self[3] ** 2,
        )
        pitch = np.arcsin(2 * (self[1] * self[3] + self[0] * self[1]))
        yaw = np.arctan2(
            -2 * (self[1] * self[2] - self[0] * self[3]),
            self[0] ** 2 + self[1] ** 2 - self[2] ** 2 - self[3] ** 2,
        )
        return roll, pitch, yaw
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Quaternion):
            w = (
                self._q[0] * other._q[0]
                - self._q[1] * other._q[1]
                - self._q[2] * other._q[2]
                - self._q[3] * other._q[3]
            )
            x = (
                self._q[0] * other._q[1]
                + self._q[1] * other._q[0]
                + self._q[2] * other._q[3]
                - self._q[3] * other._q[2]
            )
            y = (
                self._q[0] * other._q[2]
                - self._q[1] * other._q[3]
                + self._q[2] * other._q[0]
                + self._q[3] * other._q[1]
            )
            z = (
                self._q[0] * other._q[3]
                + self._q[1] * other._q[2]
                - self._q[2] * other._q[1]
                + self._q[3] * other._q[0]
            )
